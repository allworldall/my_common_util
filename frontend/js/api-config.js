...
window.APP_CONFIG = {
  apiBase: "",
  apiToken: "",
  /** PDF 拆分上传大小上限（字节），需与后端 pdf.split.max-file-size-bytes 保持一致 */
  pdfMaxFileSizeBytes: 50 * 1024 * 1024,
  /** PDF 压缩上传大小上限（字节），需与后端 pdf.compress.max-file-size-bytes 保持一致 */
  pdfCompressMaxFileSizeBytes: 100 * 1024 * 1024,
